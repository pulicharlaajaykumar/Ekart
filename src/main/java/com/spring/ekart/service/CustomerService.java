...
import com.spring.ekart.dto.Cart;
...
import com.spring.ekart.repository.CustomerRepository;
import com.spring.ekart.repository.ItemRepository;
import com.spring.ekart.repository.OrderRepository;
...
public class CustomerService {
	...
	public String searchProducts(HttpSession session, String query, ModelMap map) {
		if (session.getAttribute("customer") != null) {
			...
			List<Product> list1 = productRepository.findByNameLikeAndApprovedTrue(toSearch);
			List<Product> list2 = productRepository.findByDescriptionLikeAndApprovedTrue(toSearch);
			List<Product> list3 = productRepository.findByCategoryLikeAndApprovedTrue(toSearch);
	...
	public String viewOrders(HttpSession session, ModelMap map) {
		if (session.getAttribute("customer") != null) {
			...
			List<com.spring.ekart.dto.Order> orders = orderRepository.findByCustomer(customer);
			if (orders.isEmpty()) {
	...
	public String payment(HttpSession session, ModelMap map) {
		...
		if (session.getAttribute("customer") != null) {
			try {
				...
				//cart logic after updating products............
	...
	public String decrease(int id, HttpSession session) {
		if (session.getAttribute("customer") != null) {
			...
			Product product = productRepository.findByNameLikeAndApprovedTrue(item.getName()).get(0);
